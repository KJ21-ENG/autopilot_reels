...
- [ ] [AI-Review][High] Add assertions that no processing occurs when webhook signature verification fails (e.g., `insert` and `listLineItems` not called). (AC #1)
- [ ] [AI-Review][High] Document security constraints or edge cases discovered during verification testing in Dev Notes or Completion Notes.
...
- Security/edge-case notes: Invalid webhook signatures should return a 400 error and must not trigger any processing side effects (no Stripe line item fetches or payment inserts). This is asserted in verification tests.
