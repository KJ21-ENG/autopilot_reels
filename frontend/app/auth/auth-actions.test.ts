...
const buildSupabaseMock = (overrides?: Partial<SupabaseAuthMock>): SupabaseMock => {
...
describe("auth actions", () => {
  it("redirects after successful email sign-in", async () => {
    ...
    const result = await performEmailAuth({
      supabase: supabase as unknown as SupabaseMock,
    ...
    expect(fetchFn).toHaveBeenCalledWith("/api/auth/session", expect.any(Object));
  ...
  it("returns a friendly error when credentials are invalid", async () => {
    ...
    expect(result.error).toBe('No account found with "user@example.com"');
  });
  ...
  it("blocks post-payment sign-up when entered email does not match checkout email", async () => {
    ...
    const fetchFn = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ data: { payment_found: true, match: false }, error: null }),
    });
    ...
    expect(result.error).toBe("Use the same email you used during checkout to create your account.");
    expect(signUp).not.toHaveBeenCalled();
  });
  ...
  it("shows Google login guidance only when existing account has Google provider", async () => {
    ...
    const fetchFn = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ data: { has_google_provider: true }, error: null }),
    });
  ...
  it("keeps generic sign-in guidance when existing account is not Google-linked", async () => {
    ...
    const fetchFn = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ data: { has_google_provider: false }, error: null }),
    });
    ...
    expect(result.error).toBe("That email already has an account. Please sign in instead.");
  });
