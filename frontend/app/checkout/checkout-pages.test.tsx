...
describe("checkout result pages", () => {
    ...
    it("links success and cancel pages to the correct destinations", async () => {
        const successMarkup = await renderSuccess("?session_id=cs_test_456");
        const cancelMarkup = await renderCancel();

        expect(successMarkup).toContain(
            'href="/auth?redirect=/dashboard&amp;post_payment=1&amp;session_id=cs_test_456"',
        );
    ...
    it("hides the main confirmation when rendered in the auth-only tab", async () => {
        ...
        expect(markup).not.toContain(
            "Your plan is active. Let’s finish setup.",
        );
    });
