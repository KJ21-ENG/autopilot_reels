...
async function sendResumeEmail({
    ...
    if (!apiKey || !fromEmail) {
        ...
        console.info("Resume setup link generated.", {
            email,
            resumeUrl,
            missing,
        });
    ...
    if (!response.ok) {
        ...
        console.error("Resend API Error (Resume Link):", errorData);
...
export async function POST(request: Request) {
    ...
    if (!email && !sessionId) {
        return jsonError(
            400,
            "missing_email",
            "Email or session ID is required.",
        );
    }
    ...
    if (!secret) {
        return jsonError(
            500,
            "missing_config",
            "Resume setup is not configured.",
        );
    }
    ...
    if (paymentError) {
        console.warn(
            "Unable to resolve paid payment for resume setup.",
            paymentError,
        );
    ...
    if (
        ...
        payments.length === 0 ||
        !payments[0]?.id ||
        !payments[0]?.stripe_session_id
    ) {
        return NextResponse.json({
            data: { status: "no_paid_payment" },
            error: null,
        });
    }
    ...
    if (linksError) {
        console.warn(
            "Unable to resolve payment linkage status for resume setup.",
            linksError,
        );
        return jsonError(500, "resume_failed", "Unable to process request.");
    }
    ...
    if (!resolvedEmail) {
        return NextResponse.json({
            data: { status: "no_email_on_payment" },
            error: null,
        });
    }
    ...
    try {
        const delivery = await sendResumeEmail({
            email: resolvedEmail,
            resumeUrl,
        });
        if (delivery === "log") {
        ...
        return NextResponse.json({
            data: { status: "email_sent", delivery },
            error: null,
        });
    } catch (error) {
